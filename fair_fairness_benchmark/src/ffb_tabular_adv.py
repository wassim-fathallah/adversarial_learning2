...
if __name__ == '__main__':
    ...
    numerical_cols = X_train.select_dtypes("float32").columns
    ...
    for epoch in range(args.clf_num_epochs):
        for x, y, _ in train_loader:
            ...
            clf.zero_grad()
            h, p_y = clf(x)
            loss = clf_criterion(p_y.clamp(1e-7, 1 - 1e-7), y)
            loss.backward()
            clf_optimizer.step()
    ...
    for epoch in range(args.adv_num_epochs):
        for x, _, z in train_loader:
            ...
            y_hat = y_hat.detach()
            adv.zero_grad()
            p_z = adv(y_hat).clamp(1e-7, 1 - 1e-7)
    ...
    def train(clf, adv, data_loader, clf_criterion, adv_criterion, clf_optimizer, adv_optimizer):
        ...
        for x, y, z in data_loader:
            ...
            p_y = p_y.detach()
            adv.zero_grad()
            p_z = adv(p_y).clamp(1e-7, 1 - 1e-7)
            loss_adv = args.lam * adv_criterion(p_z, z)
            loss_adv.backward()
            adv_optimizer.step()

        # Train classifier (full epoch)
        for x, y, z in data_loader:
            x = x.to(device)
            y = y.to(device)
            z = z.to(device)
            ...
            p_z = adv(p_y).clamp(1e-7, 1 - 1e-7)
            loss_adv = args.lam * adv_criterion(p_z, z)
            clf_loss = clf_criterion(p_y.clamp(1e-7, 1 - 1e-7), y) - loss_adv
            clf_loss.backward()
            clf_optimizer.step()
